...
def train_epoch(model: torch.nn.Module, optimizer, train_batches):
    model.train()
    running_loss = 0
...
def train_run(model, optimizer, train_batches, valid_batches, do_validation=False):
    ...
    for epoch in iterator:
        loss = train_epoch(model, optimizer, train_batches)
        loss_all += [loss]
...
if __name__ == "__main__":
    ...
    pickle.dump(train_stop - train_start, open(os.path.join(PATH, "train_time.pickle"), "wb"))
