...
experiment_loop() {
    ...
    for n_size in ${_neighbourhood_sizes[@]}; do
        for train_run_idx in 0; do
            new_path="$path$n_size/$train_run_idx"
            mkdir -p $new_path
            echo $new_path
            python training_script_classification.py --neighbourhood $n_size --epochs 50 --model $model --dataset "elliptic++" --device cuda:0 --path $new_path --lr $lr &
...
# models=("sage_attn")
models=("gat" "sage" "sage_attn")
for model in ${models[@]}; do
    path=""./experiments/"model_checkpoints_final_test/${model^^}"
    mkdir -p $path
